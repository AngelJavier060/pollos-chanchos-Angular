...
export class TypeFoodService {
  ...
  createTypeFood(typeFood: TypeFood): Observable<TypeFood> {
    return this.http.post<TypeFood>(this.apiUrl, typeFood, this.httpOptions).pipe(
      retry(1),
      catchError(this.handleError)
    );
  }

  updateTypeFood(typeFood: TypeFood): Observable<TypeFood> {
    return this.http.put<TypeFood>(`${this.apiUrl}`, typeFood, this.httpOptions).pipe(
      retry(1),
      catchError(this.handleError)
    );
  }

  deleteTypeFood(id: number): Observable<void> {
