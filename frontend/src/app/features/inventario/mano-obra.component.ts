...
@Component({
  ...
  <div class="p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-blue-800">Mano de Obra</h1>
        <p class="text-gray-600">Registro de horas y costos por trabajador</p>
      </div>
    </div>
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" formControlName="nombreTrabajador" class="w-full p-2 border rounded-md" placeholder="Ejemplo: Juan Pérez" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Cargo</label>
            <input type="text" formControlName="cargo" class="w-full p-2 border rounded-md" placeholder="Ejemplo: Operador, Cuidador, Técnico…" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Horas trabajadas</label>
            <input type="number" min="0" step="0.01" formControlName="horasTrabajadas" class="w-full p-2 border rounded-md" placeholder="Ejemplo: 8 horas" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Costo por mes</label>
            <input type="number" min="0" step="0.01" formControlName="costoPorHora" class="w-full p-2 border rounded-md" />
          </div>
          <div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Trabajador</th>
              ...
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Costo mes</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr *ngFor="let r of registros">
              ...
              <td class="px-4 py-2 text-sm">{{ r.nombreTrabajador }}</td>
              <td class="px-4 py-2 text-sm">{{ r.cargo }}</td>
              <td class="px-4 py-2 text-sm">{{ r.horasTrabajadas }}</td>
              <td class="px-4 py-2 text-sm">$ {{ r.costoPorHora | number:'1.2-2' }}</td>
...
export class ManoObraComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const basePayload = {
      ...
      horasTrabajadas: Number(v.horasTrabajadas || 0),
      costoPorHora: Number(v.costoPorHora || 0),
      fecha: v.fecha,
